...
int TMVAClassification( TString myMethodList = "" )
{
   ...
   TString fname = "mva-addition/trainStripped.root";//"./tmva_class_example.root";
   ...
   TTree* B0Signal = (TTree*)input->Get("B0Signal");
   TString sigcut = "B0_isSignal";
   TString bkgcut = "!B0_isSignal";
   TFile * inputsig = new TFile( "mva-addition/signal.root","recreate" );
   TTree *signalTree     = B0Signal->CopyTree(sigcut); //(TTree*)input->Get("TreeS");
   TFile * inputbkg = new TFile( "mva-addition/bkg.root","recreate" );
   TTree *background     = B0Signal->CopyTree(bkgcut);//(TTree*)input->Get("TreeB");
   ...
   dataloader->AddVariable( "B0_CosTBTO",'F');
   dataloader->AddVariable( "B0_cc2",'F');
   dataloader->AddVariable( "B0_hso12",'F');
   dataloader->AddVariable( "B0_R2",'F');
   dataloader->AddVariable( "B0_hso10",'F');
   dataloader->AddVariable( "B0_hso14",'F');
   dataloader->AddVariable( "B0_cc9",'F');
   dataloader->AddVariable( "B0_CosTBz",'F');
   dataloader->AddVariable( "B0_ThrustO",'F');
   dataloader->AddVariable( "B0_ThrustB",'F');
...
//   dataloader->AddSpectator( "spec2 := var1*3",  "Spectator 2", "units", 'F' );
   ...
   dataloader->PrepareTrainingAndTestTree( mycuts, mycutb,
                                        "nTrain_Signal=1000:nTrain_Background=1500:SplitMode=Random:NormMode=NumEvents:!V" );
